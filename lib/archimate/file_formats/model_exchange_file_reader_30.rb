  ...
  module FileFormats
    class ModelExchangeFileReader30
      ...
      def parse_diagrams_30(model)
        model.css(">views>diagrams>view").map do |i|
          nodes = parse_nodes(i)
      ...
      def parse_nodes(node)
        ...
        node.css("> node").map do |i|
          DataModel::ViewNode.new(
            id: identifier_to_id(i["identifier"]),
            type: i.attr(type_attr),
            model: nil,
            name: parse_lang_string(i.at_css(">label")),
            target_connections: [], # TODO: needed? "targetConnections",
            archimate_element: identifier_to_id(i[element_ref]),
      ...
      def parse_connections(node)
        ...
        node.css("> connection").map do |i|
          DataModel::Connection.new(
            ...
            relationship: identifier_to_id(i[relationship_ref]),
